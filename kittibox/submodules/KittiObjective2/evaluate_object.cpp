...
const int32_t N_MAXIMAGES = 4247;
const int32_t N_TESTIMAGES = 270;
...
vector<tDetection> loadDetections(string file_name, bool &compute_aos, bool &eval_car, bool &eval_pedestrian, bool &eval_cyclist, bool &success) {
  ...
  std::ifstream file_test(file_name.c_str());
  bool theTEST = is_empty(file_test)==0;
  if (theTEST==0){
    tDetection d;
    d.box.type = file_name;
    d.box.x1 = 0;
    d.box.y1 = 0;
    d.box.x2 = 0;
    d.box.y2 =0;
...
inline double boxoverlap(tBox a, tBox b, int32_t criterion=-1){
  ...
  if(w<=0 || h<=0)
   // std::cout<<"invalid etries"<<std::endl;
    return 0;
...
void cleanData(CLASSES current_class, const vector<tGroundtruth> &gt, const vector<tDetection> &det, vector<int32_t> &ignored_gt, vector<tGroundtruth> &dc, vector<int32_t> &ignored_det, int32_t &n_gt, DIFFICULTY difficulty){
  //std::cout<<"---------clean data--- grounturth size---"<<gt.size()<<std::endl;
  //std::cout<<"---------clean data ---detection size ---"<<det.size()<<std::endl;
  ...
  for(int32_t i=0;i<gt.size(); i++){
        ...
        {  valid_class = 1;
          //std::cout<<"-----checking box type of gt/settig valid case to 1 for------"<< std::cout<<gt[i].box.type.c_str()<<std::endl;
        }
...
tPrData computeStatistics(CLASSES current_class, const vector<tGroundtruth> &gt, const vector<tDetection> &det, const vector<tGroundtruth> &dc, const vector<int32_t> &ignored_gt, const vector<int32_t>  &ignored_det, bool compute_fp, bool compute_aos=false, double thresh=0, bool debug=false){
  ///
  //int notIgored = std::count(std::begin(ignored_gt), std::end(ignored_gt), 0);
  ...
  //    for(int32_t j=0; j<det.size(); j++){
     //std::cout<<"loopting through "<<det.size()<<" detections for overlap"<<std::endl;
    for(int32_t j=0; j<det.size(); j++){
      ...
      if(ignored_threshold[j])
        continue;
      //std::cout<<"first filter passed"<<std::endl;
   ...
   // std::cout<<"end of ovelap loop"<<std::endl;
   //  =======================================================================
...
bool eval_class (FILE *fp_det, FILE *fp_ori, CLASSES current_class,const vector< vector<tGroundtruth> > &groundtruth,const vector< vector<tDetection> > &detections, bool compute_aos, vector<double> &precision, vector<double> &aos, DIFFICULTY difficulty) {
  ...
  int N_TESTIMAGES_real = groundtruth.size();

  for (int32_t i=0; i<N_TESTIMAGES_real; i++){
...
bool eval(string path, string path_to_gt, Mail* mail){
    ...
    for (int32_t i=0; i<N_MAXIMAGES; i++) {
        ...
        // currently not in evaluaion -- skip
        if(!exists_test0(result_dir + "/" + file_name)) {
          continue;
        }

        //std::cout<<gt_dir<<std::endl;
       // std::cout<<"sfdddd"<<std::endl;

        // read ground truth and result poses
        bool gt_success,det_success;
        // std::cout<<gt_dir + "/" + file_name<<std::endl;
    ...
    // ERROR IN EVAL_CLASS - TEST IMAGES SIZE OT CORRECT 465<->481
        // std::cout<<groundtruth.size()<<std::endl;
    // eval cars
    if(eval_car){
      std::cout<<"car evaluation"<<std::endl;
      ...
      if(  
       !eval_class(fp_det,fp_ori,CAR,groundtruth,detections,compute_aos,precision[0],aos[0],EASY)
    ...
    // eval pedestrians
    if(eval_pedestrian){
      std::cout<<"pedestrians evaluation"<<std::endl;
    ...
    // eval cyclists
    if(eval_cyclist){
      std::cout<<"cyclist evaluation"<<std::endl;
