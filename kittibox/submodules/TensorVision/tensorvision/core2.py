...
def build_tensors_in_checkpoint_file(loaded_tensors):
    ...
    for i, tensor_name in enumerate(loaded_tensors[0]):
	    ...
	    not_found_tensors.append(tensor_name)
        full_var_list.append(tensor_aux)
    return full_var_list, not_found_tensors

def load_weights(checkpoint_dir, sess, saver,orig=True):
    """
    ...
    #print("weights loaded succseffully")
    CHECKPOINT_NAME = '/home/asus/code/Deep_learning_project/KittiBox/RUNS/KittiBox/KittiBox/KittiBox_pretrained/model.ckpt-179999'

    restored_vars  = get_tensors_in_checkpoint_file(file_name=CHECKPOINT_NAME)
    tensors_to_load, not_found_tensors = build_tensors_in_checkpoint_file(restored_vars)
    print("----------------------------------NOT FOUND:--------------------------------")
    print(not_found_tensors) 
   # init_custom_op = tf.variables_initializer(var_list=[not_found_tensors[0]])
...
def get_intermediate_maps(hypes,modules,image):
    with tf.name_scope("Visualize"):
        dict = modules['arch'].inference(hypes,image,train=False)
	print(dict.keys()) 

def start_tv_session(logdir):
    """
    Run one evaluation against the full epoch of data.
    ...
    saver = tf.train.Saver()
    sess = tf.Session()
    logdir = '/home/ava/KittiBox/RUNS/KittiBox/KittiBox/KittiBox_pretrained/'
