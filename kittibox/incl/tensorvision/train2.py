...
def run_training(hypes, modules, tv_graph, tv_sess, start_step=0):
    ...
    for step in xrange(start_step, hypes['solver']['max_steps']):
        ...
        # Write the summaries and print an overview fairly often.
           ...
           # print("RUNNING EVAL OPS--------------------------------")
            log_loss.append([step, loss_value])
            
	    eval_results = sess.run(eval_ops, feed_dict=feed_dict)
            np.savetxt('/home/asus/loss_log.txt',log_loss,fmt='%.2f',delimiter='\t')
    ...
    np.savetxt('/home/anna.deichler/loss_log.txt',log_loss,fmt='%d')
